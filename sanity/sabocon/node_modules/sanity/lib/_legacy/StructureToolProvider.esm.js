...
import { Box, rem, Flex, Inline, Text, useToast, Card, Stack, ErrorBoundary, Layer, useForwardedRef, BoundaryElementProvider, MenuDivider, Menu, TextSkeleton, rgba, useElementRect, LayerProvider, _raf2, TextInput, TabList, TabPanel, Code, Heading, focusFirstDescendant, Container, Spinner, usePortal, PortalProvider, Popover, useLayer, useClickOutside, useGlobalKeyDown, Grid, Button as Button$1, Dialog as Dialog$1, AvatarStack, Badge, Hotkeys, DialogProvider } from "@sanity/ui";
import { isValidElement, forwardRef, useContext, useMemo, useCallback, useId, useState, useEffect, memo, useRef, useLayoutEffect, createElement, Fragment as Fragment$1, lazy } from "react";
...
import { isSanityDocument as isSanityDocument$1, isValidationErrorMarker, isKeySegment, isIndexSegment, isReferenceSchemaType } from "@sanity/types";
...
function CommentsProviderWrapper(props) {
  const { children, documentId, documentType } = props, { enabled } = useCommentsEnabled(), { connectionState, onPathOpen, inspector, openInspector } = useDocumentPane(), { params, setParams, createPathWithParams } = usePaneRouter$1(), selectedCommentId = params == null ? void 0 : params.comment, paramsRef = useRef(params);
  useLayoutEffect(() => {
...
const DocumentPaneProvider = memo((props) => {
  const { children, index, pane, paneKey, onFocusPath } = props, schema = useSchema(), templates = useTemplates(), {
    ...
    title = null,
    views: viewsProp = []
  } = pane, paneOptions = useUnique(options), documentIdRaw = paneOptions.id, documentId = getPublishedId(documentIdRaw), documentType = options.type, params = useUnique(paneRouter.params) || EMPTY_PARAMS$1, panePayload = useUnique(paneRouter.payload), { templateName, templateParams } = useMemo(
...
function DocumentSheetListPane() {
  return /* @__PURE__ */ jsx("div", { "data-testid": "document-sheet-list-pane", children: /* @__PURE__ */ jsx(Text, { children: "SheetList view" }) });
}
...
const PaneContainer = memo(function(props) {
  const { index, isSelected, pane, paneKey } = props, { name: parentSourceName } = useSource(), {
    ...
    menuItems,
    options
  } = pane, { defaultOrdering = EMPTY_ARRAY$3, filter: filter2 } = options, params = useShallowUnique(options.params || EMPTY_RECORD), sourceName = pane.source, typeName = useMemo(() => {
    ...
    defaultLayout
  ), { title } = useI18nText(pane), defaultSortOrder = useMemo(() => (defaultOrdering == null ? void 0 : defaultOrdering.length) > 0 ? { by: defaultOrdering } : DEFAULT_ORDERING, [defaultOrdering]), [sortOrderRaw, setSortOrder] = useStructureToolSetting(
    "sort-order",
    typeName != null ? typeName : pane.id,
    //pane.id for anything that is not documentTypeList
    defaultSortOrder
  ...
  ), isSheetListLayout = layout === "sheetList", paneLayout = isSheetListLayout ? /* @__PURE__ */ jsx(DocumentSheetListPane, {}) : /* @__PURE__ */ jsx(DocumentListPane, { ...props, sortOrder: sortOrderRaw, layout });
...
`, DocumentListPane = memo(function(props) {
  const { childItemId, isActive, pane, paneKey, sortOrder: sortOrderRaw, layout } = props, schema = useSchema(), { displayOptions, options } = pane, { apiVersion, filter: filter2 } = options, params = useShallowUnique(options.params || EMPTY_RECORD), typeName = useMemo(() => {
    const staticTypes = findStaticTypesInFilter(filter2, params);
    return (staticTypes == null ? void 0 : staticTypes.length) === 1 ? staticTypes[0] : null;
...
function parseChunks(chunks, initial) {
  ...
  return chunks.reduce((pane, chunk) => {
    ...
    } else isPayloadLike(chunk) ? pane.payload = tryParseBase64Payload(chunk) : console.warn("Unknown pane segment: %s - skipping", chunk);
    return pane;
  }, sibling);
}
...
export {
  usePaneLayout as $,
  maybeSerializeInitialValueTemplateItem as A,
  maybeSerializeMenuItem as B,
  ComponentBuilder as C,
  DEFAULT_INTENT_HANDLER as D,
  maybeSerializeMenuItemGroup as E,
  FormViewBuilder as F,
  GenericListBuilder as G,
  HELP_URL as H,
  InitialValueTemplateItemBuilder as I,
  maybeSerializeView as J,
  menuItemsFromInitialValueTemplateItems as K,
  ListBuilder as L,
  MenuItemBuilder as M,
  shallowIntentChecker as N,
  structureLocaleNamespace as O,
  PaneLayout as P,
  useStructureTool as Q,
  useDocumentPane as R,
  StructureToolProvider as S,
  useDocumentTitle as T,
  usePaneRouter as U,
  Pane as V,
  PaneContent as W,
  usePane as X,
  ...
  _DEBUG as a1,
  ChildLink as a2,
  ReferenceChildLink as a3,
  ParameterizedLink as a4,
  LOADING_PANE as a5,
  LoadingPane as a6,
  PaneContainer as a7,
  ...
  SerializeError as n,
  component as o,
  createStructureBuilder as p,
  defaultInitialValueTemplateItems as q,
  defaultIntentChecker as r,
  structureTool as s,
  documentFromEditor as t,
  documentFromEditorWithInitialValue as u,
  form as v,
  getOrderingMenuItem as w,
  getOrderingMenuItemsForSchemaType as x,
  getTypeNamesFromFilter as y,
  isDocumentListItem as z
};
//# sourceMappingURL=StructureToolProvider.esm.js.map
