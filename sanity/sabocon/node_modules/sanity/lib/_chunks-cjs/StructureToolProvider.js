"use strict";
var jsxRuntime = require("react/jsx-runtime"), ui = require("@sanity/ui"), React = require("react"), _singletons = require("sanity/_singletons"), PathUtils = require("@sanity/util/paths"), sanity = require("sanity"), TooltipDelayGroupProvider = require("./TooltipDelayGroupProvider.js"), styledComponents = require("styled-components"), rxjs = require("rxjs"), operators = require("rxjs/operators"), structure = require("sanity/structure"), isHotkeyEsm = require("is-hotkey-esm"), reactI18next = require("react-i18next"), JSONInspector = require("@rexxars/react-json-inspector"), LRU = require("quick-lru"), icons = require("@sanity/icons"), router$1 = require("sanity/router"), reactRx = require("react-rx"), framerMotion = require("framer-motion"), _internal = require("@sanity/schema/_internal"), omit = require("lodash/omit.js"), throttle = require("lodash/throttle.js"), deepEquals = require("react-fast-compare"), types = require("@sanity/types"), groqJs = require("groq-js"), isEqual = require("lodash/isEqual.js"), shallowEquals = require("shallow-equals"), rxjsExhaustmapWithTrailing = require("rxjs-exhaustmap-with-trailing"), CopyToClipboard = require("react-copy-to-clipboard"), isNumber = require("lodash/isNumber.js"), isString = require("lodash/isString.js"), uniqBy = require("lodash/uniqBy.js"), negate = require("lodash/negate.js"), uuid = require("@sanity/uuid"), react = require("@sanity/telemetry/react"), telemetry = require("@sanity/telemetry"), uniqueId = require("lodash/uniqueId.js"), reactIs = require("react-is"), camelCase = require("lodash/camelCase.js"), speakingurl = require("speakingurl"), uniq = require("lodash/uniq.js"), kebabCase = require("lodash/kebabCase.js"), generateHelpUrl_esm = require("./generate-help-url.esm.js"), find = require("lodash/find.js"), startCase = require("lodash/startCase.js");
...
function usePaneLayout() {
  const pane = React.useContext(_singletons.PaneLayoutContext);
  if (!pane)
...
function usePane() {
  const pane = React.useContext(_singletons.PaneContext);
  if (!pane)
...
function CommentsProviderWrapper(props) {
  const { children, documentId, documentType } = props, { enabled } = sanity.useCommentsEnabled(), { connectionState, onPathOpen, inspector, openInspector } = useDocumentPane(), { params, setParams, createPathWithParams } = structure.usePaneRouter(), selectedCommentId = params == null ? void 0 : params.comment, paramsRef = React.useRef(params);
...
const DocumentPaneProvider = React.memo((props) => {
  const { children, index, pane, paneKey, onFocusPath } = props, schema = sanity.useSchema(), templates = sanity.useTemplates(), {
    __internal_tasks,
    document: {
    ...
    title = null,
    views: viewsProp = []
  } = pane, paneOptions = sanity.useUnique(options), documentIdRaw = paneOptions.id, documentId = sanity.getPublishedId(documentIdRaw), documentType = options.type, params = sanity.useUnique(paneRouter.params) || EMPTY_PARAMS$1, panePayload = sanity.useUnique(paneRouter.payload), { templateName, templateParams } = React.useMemo(
...
function DocumentSheetListPane() {
  return /* @__PURE__ */ jsxRuntime.jsx("div", { "data-testid": "document-sheet-list-pane", children: /* @__PURE__ */ jsxRuntime.jsx(ui.Text, { children: "SheetList view" }) });
}
...
const PaneContainer = React.memo(function(props) {
  const { index, isSelected, pane, paneKey } = props, { name: parentSourceName } = sanity.useSource(), {
    defaultLayout = "default",
    initialValueTemplates = sanity.EMPTY_ARRAY,
    menuItemGroups,
    menuItems,
    options
  } = pane, { defaultOrdering = sanity.EMPTY_ARRAY, filter } = options, params = useShallowUnique(options.params || EMPTY_RECORD), sourceName = pane.source, typeName = React.useMemo(() => {
    ...
    defaultLayout
  ), { title } = sanity.useI18nText(pane), defaultSortOrder = React.useMemo(() => (defaultOrdering == null ? void 0 : defaultOrdering.length) > 0 ? { by: defaultOrdering } : DEFAULT_ORDERING, [defaultOrdering]), [sortOrderRaw, setSortOrder] = useStructureToolSetting(
    "sort-order",
    typeName != null ? typeName : pane.id,
    //pane.id for anything that is not documentTypeList
    defaultSortOrder
  ...
  ), isSheetListLayout = layout === "sheetList", paneLayout = isSheetListLayout ? /* @__PURE__ */ jsxRuntime.jsx(DocumentSheetListPane, {}) : /* @__PURE__ */ jsxRuntime.jsx(DocumentListPane, { ...props, sortOrder: sortOrderRaw, layout });
...
`, DocumentListPane = React.memo(function(props) {
  const { childItemId, isActive, pane, paneKey, sortOrder: sortOrderRaw, layout } = props, schema = sanity.useSchema(), { displayOptions, options } = pane, { apiVersion, filter } = options, params = useShallowUnique(options.params || EMPTY_RECORD), typeName = React.useMemo(() => {
    const staticTypes = findStaticTypesInFilter(filter, params);
    return (staticTypes == null ? void 0 : staticTypes.length) === 1 ? staticTypes[0] : null;
